...
export function Calculator({ priceList, isAdmin = false }: { priceList: PriceList; isAdmin?: boolean }) {
  ...
  const setProductType = (type: ProductType) => {
    if (type === 'steel') {
      ...
      update({ productType: type, horizontalPanel: false, felt: false, tile: false, gutters: false, sheet: 'ocynk', sheetColor: undefined, partitionWalls: [], openwork: { ...input.openwork, mode: 'none' } });
    }
  };
