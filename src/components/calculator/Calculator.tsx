...
export function Calculator({ priceList, isAdmin = false }: { priceList: PriceList; isAdmin?: boolean }) {
  ...
  const setProductType = (type: ProductType) => {
    if (type === 'steel') {
      ...
      update({ productType: type, horizontalPanel: false, felt: false, tile: false, gutters: false, sheet: 'ocynk', sheetColor: undefined, flashingColor: undefined, gates: input.gates.map((gate) => ({ ...gate, color: undefined })), doorColors: undefined, windows: input.windows.map((window) => ({ ...window, color: undefined })), partitionWalls: [], openwork: { ...input.openwork, mode: 'none' } });
    }
  };
